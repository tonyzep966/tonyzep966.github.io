...
def zip(*iterables):
    ...
    while iterators:
        ...
        yield tuple(result)
```
`zip()`先将参数转化为元组（星号表达式），之后再转换为包含每一个可迭代参数的迭代器的列表。然后依次迭代一次，如果迭代出的结果不为空（判断elem是否为sentinel）则将该结果加入result列表; 如果为空，便直接返回空值，这样你不必担心传入该函数的可迭代对象元素个数不相等，因为`zip()`函数这么做就会舍弃掉不相等个数之后的元素，如果你希望保留那些元素，可以考虑使用`itertools.zip_longest()`。在函数的最后，可以看到函数使用yield返回一个result的元组，这是为迭代提供了便利。同时，我们也知道了`zip（）`函数其实是一个“生成器”，而这些关于yield的学习，我想会在下一篇博客中进行。我也将对星号表达式进行学习与总结。